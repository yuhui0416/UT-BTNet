...
def getTopoLoss(likelihood_tensor, gt_tensor, t ):
    """
    ...
    for frame in range(0, likelihood.shape[0]):
        ...
        for y in range(0, likelihood.shape[1], topo_size):
            for x in range(0, likelihood.shape[2], topo_size):
                ...
                lh_patch = likelihood[frame][y:min(y + topo_size, likelihood.shape[1]),
                             x:min(x + topo_size, likelihood.shape[2])]
                gt_patch = gt[frame][y:min(y + topo_size, gt.shape[1]),
                ...
                if (len(idx_holes_to_fix) > 0 or len(idx_holes_to_remove) > 0):
                    for hole_indx in idx_holes_to_fix:
                        if (int(bcp_lh[hole_indx][0]) >= 0 and int(bcp_lh[hole_indx][0]) < likelihood.shape[1] and int(
                                bcp_lh[hole_indx][1]) >= 0 and int(bcp_lh[hole_indx][1]) < likelihood.shape[2]):
                            topo_cp_weight_map[frame][y + int(bcp_lh[hole_indx][0]), x + int(
                                bcp_lh[hole_indx][1])] = 1  # push birth to 0 i.e. min birth prob or likelihood
                            topo_cp_ref_map[frame][y + int(bcp_lh[hole_indx][0]), x + int(bcp_lh[hole_indx][1])] = 0
                        if (int(dcp_lh[hole_indx][0]) >= 0 and int(dcp_lh[hole_indx][0]) < likelihood.shape[
                            1] and int(dcp_lh[hole_indx][1]) >= 0 and int(dcp_lh[hole_indx][1]) <
                                likelihood.shape[2]):
                            topo_cp_weight_map[frame][y + int(dcp_lh[hole_indx][0]), x + int(
                                dcp_lh[hole_indx][1])] = 1  # push death to 1 i.e. max death prob or likelihood
                            topo_cp_ref_map[frame][y + int(dcp_lh[hole_indx][0]), x + int(dcp_lh[hole_indx][1])] = 1
                    for hole_indx in idx_holes_to_remove:
                        if (int(bcp_lh[hole_indx][0]) >= 0 and int(bcp_lh[hole_indx][0]) < likelihood.shape[
                            1] and int(bcp_lh[hole_indx][1]) >= 0 and int(bcp_lh[hole_indx][1]) <
                                likelihood.shape[2]):
                            topo_cp_weight_map[frame][y + int(bcp_lh[hole_indx][0]), x + int(
                            ...
                            if (int(dcp_lh[hole_indx][0]) >= 0 and int(dcp_lh[hole_indx][0]) < likelihood.shape[
                                1] and int(dcp_lh[hole_indx][1]) >= 0 and int(dcp_lh[hole_indx][1]) <
                                    likelihood.shape[2]):
                                topo_cp_ref_map[frame][y + int(bcp_lh[hole_indx][0]), x + int(bcp_lh[hole_indx][1])] = \
                                    likelihood[frame][int(dcp_lh[hole_indx][0]), int(dcp_lh[hole_indx][1])]
                            else:
                                topo_cp_ref_map[frame][y + int(bcp_lh[hole_indx][0]), x + int(bcp_lh[hole_indx][1])] = 1
                        if (int(dcp_lh[hole_indx][0]) >= 0 and int(dcp_lh[hole_indx][0]) < likelihood.shape[
                            1] and int(dcp_lh[hole_indx][1]) >= 0 and int(dcp_lh[hole_indx][1]) <
                                likelihood.shape[2]):
                            topo_cp_weight_map[frame][y + int(dcp_lh[hole_indx][0]), x + int(
                            ...
                            if (int(bcp_lh[hole_indx][0]) >= 0 and int(bcp_lh[hole_indx][0]) < likelihood.shape[
                                1] and int(bcp_lh[hole_indx][1]) >= 0 and int(bcp_lh[hole_indx][1]) <
                                    likelihood.shape[2]):
                                topo_cp_ref_map[frame][y + int(dcp_lh[hole_indx][0]), x + int(dcp_lh[hole_indx][1])] = \
                                    likelihood[frame][int(bcp_lh[hole_indx][0]), int(bcp_lh[hole_indx][1])]
                            else:
                                topo_cp_ref_map[frame][y + int(dcp_lh[hole_indx][0]), x + int(dcp_lh[hole_indx][1])] = 0

        # topo_cp_weight_map_patch.append(topo_cp_weight_map.copy())
        # topo_cp_ref_map_patch.append(topo_cp_ref_map.copy())
    ...
    # topo_cp_weight_map = torch.tensor(topo_cp_weight_map_patch, dtype=torch.float).cuda()
    # topo_cp_ref_map = torch.tensor(topo_cp_ref_map_patch, dtype=torch.float).cuda()
